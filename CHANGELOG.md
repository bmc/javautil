...
* org.clapper.util.classutil.ClassInfo now contains information about a class's
  methods and fields.
...
* Added two new methods to org.clapper.util.text.TextUtil:

  a) isPrintable() determines whether a character is printable. Its
     definition of printable is fairly simple, but serviceable:

         A character is assumed to be printable if (a) it's in the Basic
         Latin, Latin 1 Supplement, or Extended Latin A Unicode block, and
         (b) its type, as returned by
         <tt>java.lang.Character.getType()</tt> is one of:
         ...
         This definition of "printable" that doesn't take into account
         specific locales.

  b) charToUnicodeEscape() converts a character to a string representing
     its Java unicode escape sequence.
...
* In org.clapper.util.classutil, AndClassFilter and OrClassFilter are now
  final. This change avoids problems with the (useful) constructors for
  those classes potentially calling methods that could be overridden.

* In org.clapper.util.io, AndFileFilter, AndFilenameFilter, OrFileFilter
  and OrFilenameFilter are now final. This change avoids problems with the
  (useful) constructors for those classes potentially calling methods that
  could be overridden.

* Changes and fixes to the org.clapper.util.config.Configuration class:

  a) It how properly handles variable references to the "system", "program"
     and "env" pseudo-sections when an include file was being processed.

  b) It will now abort, by default, if it encounters a reference to a
  ...
  c) It now supports "${var?default value}" syntax. In that example, if
...
* In org.clapper.util.misc.FileHashMap:
  ...
  a) Class was using an exclusive file lock to prevent concurrent
  ...
  b) Added delete() method, to force deletion of files.
...
* The org.clapper.util.text.UnixShellVariableSubstituter class has been
  ...
  a) It now supports a "${var?default value}" syntax. In that example,
     if "${var}" does not have a value, or has an empty value, the string
     "default value" will be substituted.
  b) It can be configured to abort when a variable is undefined, rather
     than substituting an empty string.
  c) It can be configured to abort on syntax error (e.g., an unclosed
     "}"), rather than simply passing the bad variable reference through.
  d) The version of substitute() that takes an "allowEscapes" parameter
...
* The org.clapper.util.text.WindowsShellVariableSubstituter class has been
  ...
  a) It can be configured to abort when a variable is undefined, rather
     than substituting an empty string.
  b) It can be configured to abort on syntax error (e.g., an unclosed
     "}"), rather than simply passing the bad variable reference through.
...
* In org.clapper.util.text.TextUtil:
  ...
  a) Added various hexadecimal conversion routines.
  b) Added romanNumeralsForNumber() method.
...
* Deprecated MultipleRegexFilenameFilter, CombinationFilenameFilter and
  ...
  new, simpler filters that can be combined more easily:
      ...
      AndFilenameFilter    combines FilenameFilters with an AND operator
      NotFilenameFilter    negates the sense of a FilenameFilter
      OrFilenameFilter     combines FilenameFilters with an OR operator
      RegexFilenameFilter  FilenameFilter that matches file names against
                           a regular expression

      AndFileFilter        combines FileFilters with an AND operator
      NotFileFilter        negates the sense of a FileFilter
      OrFileFilter         combines FileFilters with an OR operator
      RegexFileFilter      FileFilter that matches file names against
                           a regular expression

* Created new org.clapper.io.classutil package with some class-related
  classes:

      ClassFinder               locate classes in classpath, jars, etc.
      ClassFilter               used with ClassFinder to filter classes
                                (akin to java.io.FilenameFilter)
      RegexClassFilter          ClassFilter that matches classes by
                                regular expression
      SubclassClassFilter       ClassFilter that matches classes that
                                subclass a specific class or interface
      InterfaceOnlyClassFilter  ClassFilter that matches only interfaces
      AndClassFilter            combines ClassFilters with an AND operator
      NotClassFilter            negates the sense of a ClassFilter
      OrClassFilter             combines ClassFilters with an OR operator
      ClassLoaderBuilder        helps build a java.net.URLClassLoader
      ClassUtil                 misc. utility methods
...
* The CommandLineUtility and UsageInfo classes (in org.clapper.util.cmdline)
  now permit an option to be specified with a null explanation, denoting a
  "hidden" option that isn't shown in the usage display. This is useful,
  for instance, when you've deprecated an option but are retaining it for
...
* Fixed a bug in the org.clapper.util.io.WordWrapWriter class: When writing
  a new line (i.e., when the last thing written was a newline character),
  if the first token to be written exceeded the line length, the
  WordWrapWriter.flushBuffer() method would put out a spurious newline
...
* Added two new versions of the TextUtil.join() method that allow the caller to
  specify a starting array index and length.

* RollingFileWriter was mistakenly treating embedded backslashes in its file
  patterns as Unix-style escapes. This led to problems on Windows systems.
...
* The library can now be installed (along with all dependent jars,
  ...
  graphical installer. The graphical installer is based on the
  IzPack installer framework (http://www.izforge.com/izpack/).
...
* Some additional Java 5.0 (a.k.a., JDK 1.5) changes, specifically:
  ...
  a) org.clapper.util.logging.LogLevel is now an enum
  b) org.clapper.util.io.CombinationFilterMode is now an enum
  c) org.clapper.util.misc.Semaphore and
...
* Some minor changes to the org.clapper.util.mail.EmailMessage class:
  ...
  a) The clear() method now explicitly deletes any temporary files that
  ...
  b) Added some new versions of the setText() and addAttachment() methods,
...
Version 1.1.9 (09 August, 2005) [MAINTENANCE RELEASE OF DEPRECATED CODE BRANCH]
